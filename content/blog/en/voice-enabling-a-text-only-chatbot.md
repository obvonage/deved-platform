...
There are two sample Voice API application codes and a simple text chatbot application in this tutorial,

the first Voice API application will connect to the very simple chatbot application service that simulates requests/responses which only purpose is to illustrate how an actual chatbot can be voice-enabled,

and the second sample Voice API application has the same core voice capabilities as the first one, you will update that application to integrate with your actual chatbot for requests/responses using the first one as reference.
...
* Under Answer URL, leave HTTP GET, and enter https://<host>:<port>/answer (replace <host> and <port> with the public hostname and, if necessary public port of the server where the sample Voice API application is running), e.g.\
  <https://yyyyyyyy.ngrok.io/answer>or <https://myappname.herokuapp.com/answer> (see next sections)\
  or <https://myserver2.mycompany.com:40000/answer> (deploying on your own servers)
* Under Event URL, select HTTP POST (instead of HTTP GET), and enter https://<host>:<port>/event (replace <host> and <port> with the public hostname and, if necessary public port of the server where the sample Voice API application is running), e.g.\
  <https://yyyyyyyy.ngrok.io/event>or <https://myappname.herokuapp.com/event> [ ](https://myappname.herokuapp.com/answer)(see next sections)\
  or <https://myserver2.mycompany.com:40000/event> (deploying on your own servers)
...
* The Simple Text-Only chatbot server public hostname and port (as BOT_SERVER), the argument has no http:// nor https:// prefix, no trailing /, and no sub-path, e.g.\
  xxxxxxx.ngrok.io\
  or mysimplebotname.herokuapp.com\
  or myserver1.mycompany.com:32000
...
In the first phase, we are going to use and deploy the reference application voice-on-text-bot-app-with-simple-bot.js from this [repository](https://github.com/nexmo-se/voice-enabling-text-bot-application), in conjunction with the simple chatbot application very-simple-bot.js from this [repository](https://github.com/nexmo-se/voice-enabling-text-bot-simple-chatbot). 
...
Download the very simple chatbot application code from the [repository](https://github.com/nexmo-se/voice-enabling-text-bot-simple-chatbot) to a local folder, then go to that folder.
...
On your Heroku dashboard, where your application page is shown, click on Open App button. That hostname is the one to be used under your corresponding [Vonage Voice API application Capabilities](https://dashboard.nexmo.com/applications) (click on the corresponding application, then \[Edit]).
...
* * ‘/makecall’
  * * If you would like to initiate outbound calls
    ...
    * To initiate the outbound call, in a web browser, enter the web address: <this-application-host-name>/placecall, e.g.

xxxxxxx.ngrok.io/makecall
...
* * This webhook route is called by the Vonage Voice API platform on ASR (Automatic Speech Recognition) results whether a transcript has been successfully returned or not, e.g. there could have been noises instead of actual spoken words or timed out if no sound has been detected (the time out duration corresponds to the parameter startTimeout value)
  * It starts a new ASR cycle by returning a [NCCO](https://developer.vonage.com/voice/voice-api/ncco-reference#input) with “action”: “input”
* ‘/botreply’
